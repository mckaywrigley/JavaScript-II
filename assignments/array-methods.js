...
let allCaps = runners.map(e => e.first_name.toUpperCase());
...
let largeShirts = runners.filter(e => {
    if(e.shirt_size === 'L') {
        return e;
    }
});
...
runners.forEach(e => {
    donations.push(e.donation);
});

console.log(donations);

donationTotal = (total, donation) => total + donation;

console.log(donations.reduce(donationTotal));
...
// Problem 1
// Get a list of all runner emails for a raceday checklist.
let emails = [];
runners.forEach(e => {
    emails.push(e.email);
});
...
// Problem 2
// Give those who donate over 200 a special thank you.
let special = [];
runners.filter(e => {
    if(e.donation > 200) {
        special.push(e);
...
// Problem 3
// Kwimbee has requested a list for them to highlight their employees.
let kwimbee = [];
runners.filter(e => {
    if(e.company_name === 'Kwimbee') {
        kwimbee.push(e);
    }
});
